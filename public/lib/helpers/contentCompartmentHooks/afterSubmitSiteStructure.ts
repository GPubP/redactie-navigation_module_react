...
const afterSubmitSiteStructure: ExternalCompartmentAfterSubmitFn = async (
	...
	site
		...
		const editablePosition = pathOr(false, ['meta', 'sitestructuur', 'editablePosition'])(contentType);
		const structurePosition = pathOr(PositionValues.none, ['meta', 'sitestructuur', 'structurePosition'])(contentType);
		const isEditable = (structurePosition === PositionValues.limited && editablePosition) || structurePosition === PositionValues.unlimited;

		if(!isEditable) {
			return
		}

		const pendingSiteStructureItem = await siteStructureItemsFacade.pendingSiteStructureItem$.pipe(take(1)).toPromise();

		await siteStructureItemsFacade.createSiteStructureItem(site?.uuid!, `${pendingSiteStructureItem?.treeId}`, {
		...
		}, ALERT_CONTAINER_IDS.siteStructureItemsOverview).then(() => {
