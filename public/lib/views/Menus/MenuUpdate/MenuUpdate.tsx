...
const MenuUpdate: FC<NavigationRouteProps<{ menuId?: string; siteId: string }>> = ({
	location,
	route,
	tenantId,
}) => {
	...
	const [forceNavigateToOverview] = useOnNextRender(() =>
		navigate(MODULE_PATHS.site.menusOverview, { siteId })
	);
		...
		<>
			<ContextHeader
				...
				linkProps={(props: ContextHeaderTabLinkProps) => ({
					...props,
					to: generatePath(`${MODULE_PATHS.site.menuDetail}/${props.href}`, {
						siteId,
						menuId,
					}),
					component: Link,
